...
import { Shield, ArrowLeft, Sun, Moon, Lock, Mail, Activity } from "lucide-react";
...
function AdminLoginForm() {
  ...
  const onSubmit = async (data: LoginInput) => {
    ...
    try {
      ...
      toast("Logged in successfully as Admin.", "success", "Welcome Back");
      router.push("/admin");
    } catch (err: any) {
      setShake(true);
  ...
  return (
    <div className="min-h-screen bg-[#080c16] relative flex flex-col items-center justify-center p-6 overflow-hidden">
      ...
      <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] rounded-full bg-teal-900/5 blur-[120px]" />
      ...
      <div className="absolute top-6 left-6 z-20">
        <button
          onClick={() => router.push("/")}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-800 bg-slate-900/50 hover:bg-slate-900 text-slate-400 hover:text-white text-xs transition-colors"
        >
      ...
      <motion.div
        ...
        className={`w-full max-w-md p-8 rounded-2xl border border-slate-800 bg-slate-900/40 backdrop-blur-md relative ${
          shake ? "animate-shake" : ""
        }`}
      >
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="text-slate-400 text-xs">Enter your administrative credentials to log in.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-3 rounded-lg border bg-slate-950/60 text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-teal-500/50 focus:border-teal-500/50 transition-colors ${
          ...
          <button
            ...
            className="w-full py-3 px-4 rounded-lg bg-teal-600 hover:bg-teal-500 text-white font-semibold text-sm transition-colors flex items-center justify-center gap-2 mt-2 shadow-lg shadow-teal-900/10"
          >
            {submitLoading ? (
